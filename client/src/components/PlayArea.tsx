...
export class PlayArea extends React.Component<IPlayAreaProp, IGameProp> {
  ...
  private saveState = async (): Promise<any> => {
    ...
    const request = await fetch(`http://reactships.herokuapp.com//api/gamestate/${this.state.roomID}`, {
      method: 'POST',
      headers: {
  ...
  private getState = async (): Promise<any> => {
    const request = await fetch(`http://reactships.herokuapp.com//api/gamestate/${this.props.roomid}&${this.props.player}`);
    const json = await request.json();
